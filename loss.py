...
class AUCPRHingeLoss(nn.Module):
    ...
    def __init__(self, num_classes=2, num_anchors=20, weights=None, *args, **kwargs):
        """Args:
        ...
        self.precision_values, self.delta = AUCPRHingeLoss._range_to_anchors_and_delta(
        ...
        self.biases = nn.Parameter(
            torch.FloatTensor(self.num_classes, self.num_anchors, device=device).zero_()
        )
        self.lambdas = nn.Parameter(
            torch.FloatTensor(self.num_classes, self.num_anchors, device=device).data.fill_(
                1.0
            )
        )
    ...
    @staticmethod
    def _range_to_anchors_and_delta(precision_range, num_anchors):
        ...
        return torch.from_numpy(precision_values).to(device=device), delta
    ...
    def _prepare_labels_weights(logits, targets, weights=None):
        ...
        labels = torch.FloatTensor(N, C, device=device).zero_().scatter(1, targets.unsqueeze(1).to(dtype=torch.int64).data, 1)

        if weights is None:
            weights = torch.FloatTensor(N, device=device).data.fill_(1.0)
